  ...
  #displayComponents (name) {
    ...
    if (name === 'pageTab') {
      ...
      if (this.#data.input.length > 0) lines.push(`${this.#style.selectBackground} ${this.#style.selectFont}${this.#data.input}`)
        ...
        if (os.platform() === 'linux' || os.platform() === 'darwin') lines.push(`${this.#style.notSelectBackground} ${this.#style.notSelectFont}⇦ ⇨ 切換頁面 - ⇧⇩ 滑動頁面 - 打字來輸入內容 - Enter 選取內容`)
        else if (os.platform() === 'win32') lines.push(`${this.#style.notSelectBackground} ${this.#style.notSelectFont}← → 切換頁面 -  ↑ ↓ 滑動頁面 - 打字來輸入內容 - Enter 選取內容`)
      }
