...
module.exports = async (images) => {
  let state = (plugins.Log === undefined) ? undefined : await plugins.Log.addState('white', '檢查器', '正在檢查圖片的資料 (0%)')

  await resolvePromise(images, async (item, index) => {
    if (plugins.Log !== undefined) state.change('white', '檢查器', `正在檢查圖片的資料 (${Math.trunc((100/images.length)*index)}%)`)
  ...
  if (plugins.Log !== undefined) state.change('white', '檢查器', `正在檢查是否有多餘的圖片資料`)
  ...
  if (plugins.Log !== undefined) state.finish('green', '檢查器', '圖片資料檢查完成')
}
