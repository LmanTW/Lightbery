...
module.exports = class {
  ...
  addState (color, title, content, parentState) {
    if (parentState === undefined) {
      ...
      this.#states[id] = { color: FontColor[color], title, content, children: [] }
      ...
      this.#states[parentState].children[id] = { color: FontColor[color], title, content }
  ...
  changeState (id, color, title, content) {
    if (id.includes('.')) this.#states[id.split('.')[0]].children[id.split('.')[1]] = { color: FontColor[color], title, content }
    else this.#states[id] = { color: FontColor[color], title, content, children: this.#states[id].children }
  }
    ...
    if (id.includes('.')) delete this.#states[id.split('.')[0]].children[id.split('.')[1]]
      ...
      for (let i = 0; i < this.#logs.length; i++) {
        if (typeof this.#logs[i] !== 'string' && this.#logs[i].stateID === id) {
          ...
          this.#logs[i] = `${FontColor[color]}[${title}]: ${content}`
          break
        }
      }
    }
